...
void AP33772::begin() {
    ...
    if (status.is_ready) {  // negotiation finished
        if (status.is_new_pdo) {
            if (status.is_successful) event_flag.new_neg_success = 1;
            else event_flag.new_neg_fail = 1;
        }
        else  {
            if (status.is_successful) event_flag.neg_success = 1;
            else event_flag.neg_failed = 1;
        }
    }
    sleep_ms(10);
...
void AP33772::set_voltage(uint16_t target_voltage) {
    ...
    */
    ...
    if ((exist_pps) && (pdo_data[pps_index].pps.max_voltage * 100 >= target_voltage) 
        ...
        // Step 3: Compare found PDOs voltage and PPS max voltage
        if (pdo_data[temp_index].fixed.voltage * 50 > pdo_data[pps_index].pps.max_voltage * 100) {
            ...
            index_pdo = pps_index;
            req_pps_volt = pdo_data[pps_index].pps.max_voltage * 5; // Convert units from 100mV -> 20mV
            rdo_data.pps.obj_pos = pps_index + 1;
            rdo_data.pps.op_current = pdo_data[pps_index].pps.max_current;
            rdo_data.pps.voltage = req_pps_volt;
            write_rdo();
...
void AP33772::print_pdo() {
    ...
    for (int i = 0; i < num_pdo; ++i) {
        if ((pdo_data[i].byte3 & 0xF0) == 0xC0) { // PPS PDO
            printf("PDO[%i] - PPS: %4.3fV~", i + 1, (float)pdo_data[i].pps.min_voltage * 100 / 1000);
            ...
            printf("PDO[%i] - Fixed: %4.3fV ", i +1, (float)pdo_data[i].fixed.voltage * 50 / 1000);
