...
void STUSB4500::wait_exec(){
    ...
    while(!done){
        ...
        if(data[0] & FTP_CUST_REQ) continue;
        else done = true;
    }
}
...
void STUSB4500::write_sector(uint8_t sector_num, uint8_t* data){
    ...
    uint8_t value = ((sector_num & FTP_CUST_SECT) | FTP_CUST_PWR | FTP_CUST_RST_N | FTP_CUST_REQ);
...
float STUSB4500::get_low_volt_limit(PDO_NUM pdo_num){
    float result;
    if(pdo_num == PDO_2) result = (sector[3][4] >> 4) + 5.0;
    else if(pdo_num == PDO_3) result = (sector[3][6] & 0x0f) + 5.0;
    else result = 0.0;
    return result;
}
...
float STUSB4500::get_upper_volt_limit(PDO_NUM pdo_num){
    float result;
    if(pdo_num == PDO_1) result = (sector[3][3] >> 4) + 5.0;
    else if(pdo_num == PDO_2) result = (sector[3][5] & 0x0f) + 5.0;
    else result = (sector[3][6] >> 4) + 5.0;
    return result;
}
...
uint8_t STUSB4500::get_POWER_OK_config(){
...
bool STUSB4500::is_ext_power(){
    return(sector[3][2] & 0x08) >> 3;
}

bool STUSB4500::is_usb_comm_capable(){
    return sector[3][2] & 0x01;
}

bool STUSB4500::is_power_over_5v_only(){
    return(sector[4][6] & 0x80) >> 3;
}
...
void STUSB4500::read(){
    ...
    for(int i = 0; i < NUM_OF_SECTORS; ++i){
        write_byte_to_reg(FTP_CTRL_0, FTP_CUST_PWR | FTP_CUST_RST_N); // may not be needed
...
void STUSB4500::write(bool default_mode){
    if (default_mode){
        uint8_t def_sector_1[SIZE_OF_SECTOR] = {
            0x00, 0x00, 0xb0, 0xaa, 
            0x00, 0x45, 0x00, 0x00
        };
        uint8_t def_sector_2[SIZE_OF_SECTOR] = {
            0x10, 0x40, 0x9C, 0x1C, 
            0xFF, 0x01, 0x3C, 0xDF
        };
        uint8_t def_sector_3[SIZE_OF_SECTOR] = {
            0x02, 0x40, 0x0F, 0x00, 
            0x32, 0x00, 0xFC, 0xF1
        };
        uint8_t def_sector_4[SIZE_OF_SECTOR] = {
            0x00, 0x19, 0x56, 0xAF, 
            0xF5, 0x35, 0x5F, 0x00
        };
        uint8_t def_sector_5[SIZE_OF_SECTOR] = {
            0x00, 0x4B, 0x90, 0x21, 
            0x43, 0x00, 0x40, 0xFB
        };
        ...
        for(int i = 0; i < 3; ++i){
            ...
            if (ival < 0.5) nvmi[i] = 0;
            else if (ival <= 3.0) nvmi[i] = (uint8_t)(ival * 4.0) -1;
            else{
