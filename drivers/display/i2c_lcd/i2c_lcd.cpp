#include <pico_drivers/display/i2c_lcd.hpp>

#define SLEEP_TIME_MS (5)

/*#define LCD_PRINT_DEBUG*/

I2CLCD::I2CLCD(uint8_t num_rows, uint8_t num_cols)
    : LCD(num_rows, num_cols), i2c(I2C()), is_backlight(true) {
...
void I2CLCD::write_data(uint8_t data) {
    uint8_t b =
        (MASK_RS | (is_backlight << SHIFT_BACKLIGHT) | (((data >> 4) & 0x0f) << SHIFT_DATA));
    uint8_t b_mask = b | MASK_E;
