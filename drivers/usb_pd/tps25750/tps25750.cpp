...
#include <pico_drivers/usb_pd/tps25750.hpp>
#include <pico_drivers/usb_pd/tps2575x_register_map.hpp>
...
TPS25750 *TPS25750::get_instance(uint8_t address, uint sda, uint scl, uint32_t baudrate) {
    if (inst == nullptr) {
...
bool TPS25750::read_version(uint32_t &version) { return read_uint32(USB_PD_VERSION, version); }
...
bool TPS25750::read_build_description(char *buffer, size_t length) {
    ...
    return read_ascii_string(USB_PD_BUILD_DESCRIPTION, buffer, length, REGISTER_LENGTH);
}
...
bool TPS25750::write_interrupt_clear1(const InterruptRegister &clear_bits) {
    return write_bytes(USB_PD_INT_CLEAR1, clear_bits.bytes.data(), clear_bits.bytes.size());
}
...
bool TPS25750::read_power_path_status(PowerPathStatusRegister &status) {
    return read_bytes(USB_PD_POWER_PATH_STATUS, status.bytes.data(), status.bytes.size());
}
...
bool TPS25750::read_pd_status(uint32_t &status) { return read_uint32(USB_PD_PD_STATUS, status); }

bool TPS25750::read_typec_state(uint32_t &state) { return read_uint32(USB_PD_TYPEC_STATE, state); }
...
bool TPS25750::read_active_contract_pdo(ActivePdoRegister &pdo) {
    return read_bytes(USB_PD_ACTIVE_CONTRACT_PDO, pdo.bytes.data(), pdo.bytes.size());
}
...
bool TPS25750::write_register_block(uint8_t reg, const uint8_t *buffer, size_t length) {
...
bool TPS25750::read_ascii_string(uint8_t reg, char *buffer, size_t buffer_length,
