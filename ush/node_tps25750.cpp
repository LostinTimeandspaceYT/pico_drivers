...
static void tps_read_exec_callback(struct ush_object *self,
                                   struct ush_file_descriptor const *file,
                                   int argc, char *argv[]);
static void tps_write_exec_callback(struct ush_object *self,
                                    struct ush_file_descriptor const *file,
                                    int argc, char *argv[]);
static void tps_cmd_exec_callback(struct ush_object *self,
                                  struct ush_file_descriptor const *file,
                                  int argc, char *argv[]);
...
void decode_and_print_variable_pdo(struct ush_object *self, uint32_t pdo_word) {
  ...
  float max_voltage =
      static_cast<float>((pdo_word >> 20) & 0x3FF) * 50.0f / 1000.0f;
  float current = static_cast<float>(pdo_word & 0x3FF) * 10.0f / 1000.0f;
...
void decode_and_print_apdo(struct ush_object *self, uint32_t pdo_word) {
  float min_voltage =
      static_cast<float>((pdo_word >> 8) & 0xFF) * 100.0f / 1000.0f;
  float max_voltage =
      static_cast<float>((pdo_word >> 17) & 0x1FF) * 100.0f / 1000.0f;
  float current =
      static_cast<float>(pdo_word & 0x7F) * 50.0f / 1000.0f;  // 50mA units
...
void print_pdo(struct ush_object *self, uint32_t pdo_word, size_t index) {
  ...
  ush_printf(self, "  PDO %zu (%s): 0x%08lx\r\n", index + 1,
             pdo_type_to_string(type), static_cast<unsigned long>(pdo_word));
...
static void print_pdo_list(struct ush_object *self, const uint8_t *data,
                           size_t bytes) {
  if (bytes <= 2) {
...
static void tps_write_exec_callback(struct ush_object *self,
  ...
  if (!pd->write_register_block(static_cast<uint8_t>(reg_val), buffer.data(),
                                length)) {
